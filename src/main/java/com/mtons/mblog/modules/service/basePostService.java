...
public interface basePostService {
    ...
    List<Post> find(String orderBy, int size);

    List<PostVO> toPosts(List<Post> posts);

    void buildUsers(Collection<PostVO> posts, Set<Long> uids);

    void buildGroups(Collection<PostVO> posts, Set<Integer> groupIds);

    void onPushEvent(Post post, int action);
